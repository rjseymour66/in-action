...
// Pool manages a set of resoures that can be shared safely by multiple 
// goroutines. The resource being managed must implement the io.Closer
// interface
type Pool struct {
	m	   sync.Mutex
...
// Acquire retrives a resource from the pool
...
func (p *Pool) Close() {
	...
	// resources. If we don't do this, we have a deadlock.
	close(p.resources)
